/* ─────────────────────────────────────────────────────────────────────────────
 * useTwin.js — the console's state store, as a FACTORY.
 *
 ...
 *
 * WHY A FACTORY AND NOT A MODULE SINGLETON. This used to be `create(...)` at
 * module scope, which is correct for a page and wrong for a component: two
 * <FloodTwinConsole/>s on one screen would share a timeline, a feature set and a
 * hotspot list, so moving one moved the other. Each mount now gets its own
 * store, handed down through lib/context.jsx.
 * ─────────────────────────────────────────────────────────────────────────── */
...
/**
 * @param {object} overrides  Initial state from props — `features` is MERGED
 *   over the registry defaults (so a partner can switch one layer on without
 *   having to restate the other twenty), everything else replaces.
 */
export function createTwinStore(overrides = {}) {
  ...
  return createStore((set, get) => ({
    ...
    // ── features
    features: { ...defaultState(), ...featureOverrides },
    ...
    // Prop-driven initial state, applied last so it wins over the defaults above.
    ...rest,
  }));
}
